...
use crate::{
    in_::{packets::{Compression, Packet}, types::{handshake::parse_handshake, var::int::{signed::parse_var_int, unsigned::parse_var_uint}}},
    state::ProtocolStage,
};
...
impl ProtocolParser {
    ...
    pub async fn parse_packet<R: AsyncReadExt + Unpin>(
        ...
        if let Compression::Compressed { threshold } = self.compression {
            ...
            self.parse_packet_inner(reader, packet_len as usize - id_len, id).await
        }
    }
